...
export function CompletedScreen() {
  ...
  async function fetchMissions() {
    setLoading(true);
    setError(null);
    const { data, error } = await supabase.from('missions').select('*, user_missions!inner(status)').in('user_missions.status', ['completed']);
        ...
        ListEmptyComponent={
          <View className="mt-16 items-center px-6">
            {loading ? <ActivityIndicator size="large" color="#059669" /> :
             ...
             <Text className="text-center text-gray-500">Você ainda não completou nenhuma missão.</Text>}
          </View>
        }
