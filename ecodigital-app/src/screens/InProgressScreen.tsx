...
export function InProgressScreen() {
  ...
  async function fetchMissions() {
    setLoading(true);
    setError(null);
    const { data, error } = await supabase.from('missions').select('*, user_missions!inner(status)').in('user_missions.status', ['in_progress']);
    ...
    if (error) {
      ...
      const formattedData = data.map(mission => ({
        ...
        duration: '15 min',
        status: mission.user_missions[0]?.status || 'in_progress',
        ...
        ListEmptyComponent={
          <View className="mt-16 items-center px-6">
            {loading ? <ActivityIndicator size="large" color="#059669" /> :
             ...
             <Text className="text-center text-gray-500">Você não tem missões em andamento.</Text>}
          </View>
        }
